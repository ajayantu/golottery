...
func AnalyzeResults(w http.ResponseWriter, r *http.Request) {
	...
	if reqParams.LotteryName != "" {
		...
		if _, ok := mapedData[reqParams.LotteryName]; ok {
			finalResults, err = helpers.EvaluateAllLotteries(mapedData[reqParams.LotteryName], reqParams.LotteryCodes)
			if err != nil {
				helpers.Fail(w, http.StatusInternalServerError, []helpers.FailStruct{{
					Message:    err.Error(),
		...
		finalResults, err = helpers.EvaluateAllLotteries(pdfdatas, reqParams.LotteryCodes)
		if err != nil {
			helpers.Fail(w, http.StatusInternalServerError, []helpers.FailStruct{{
				Message:    err.Error(),
				ErrorField: "evaluating all result",
